...
export function RunTaskPage({ api, onNavigate }) {
  ...
  const run = async () => { const problem = validate(true); setValidation(problem); if (problem) return; setBusy(true); setError(null); try { if (form.analyzeFirst) setAnalysis(await api.analyzeTask(form.task)); const session = form.mode === 'competition' ? await api.startCompetition({ task: form.task, workspace: form.workspace, agents: form.agents }) : await api.startExecution({ task: form.task, workspace: form.workspace }); onNavigate(`/runs/${encodeURIComponent(session.runId)}`); } catch (requestError) { setError(requestError); } finally { setBusy(false); } };
